...
import { TeacherRequestType } from "./types";

interface TeacherSubmissionFormProps {
  theme?: string;
  onSubmit?: (data: TeacherSubmissionData) => void;
  onCancel?: () => void;
}
...
function FacilityIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" className="size-6" aria-hidden>
      ...
      <path d="M9 9H15M9 12H15M9 15H13" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
    </svg>
  );
}
...
function EquipmentIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" className="size-6" aria-hidden>
      <path d="M5 19L9 15M7 13L11 17" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
      <path d="M13 5L19 11L15 15L9 9L13 5Z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" />
    </svg>
  );
}
...
function MaintenanceIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" className="size-6" aria-hidden>
      ...
      <path d="M9 5L19 15" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
      <path d="M6 18L8 16" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
    </svg>
  );
}
...
function AdministrativeIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" className="size-6" aria-hidden>
      ...
      <path d="M8 9H16M8 12H14M8 15H13" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
    </svg>
  );
}
...
export function TeacherSubmissionForm({ theme = "dark", onSubmit, onCancel }: TeacherSubmissionFormProps) {
  const [requestType, setRequestType] = useState<TeacherRequestType | "">("");
  ...
  const requestTypes: RequestTypeItem[] = [
    { value: "facility_issue", label: "Facility Issue", icon: <FacilityIcon /> },
    { value: "technology_issue", label: "Technology Issue", icon: <TechnologyIcon /> },
    { value: "equipment_issue", label: "Equipment Issue", icon: <EquipmentIcon /> },
    { value: "maintenance_request", label: "Maintenance Request", icon: <MaintenanceIcon /> },
    { value: "administrative_request", label: "Administrative Request", icon: <AdministrativeIcon /> },
  ...
  return (
    <div className={`h-full flex flex-col ${shellClass}`}>
      <div className={`p-6 border-b ${isLight ? "border-black/[0.08]" : "border-white/[0.06]"}`}>
        ...
        <p className={`text-xs mt-0.5 ${isLight ? "text-black/50" : "text-white/40"}`}>Report issues, request assistance, or share suggestions</p>
      </div>
